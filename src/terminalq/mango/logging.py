...
Standard-library only. Configures a single named logger ("terminalq") with a
...
The format mirrors the existing project convention:
    [13:02:23] terminalq.fred INFO: FRED get_series: series=CPI resolved=...
"""
...
# under this (e.g. "terminalq.fred") via `get_logger`.
_ROOT_LOGGER_NAME = "terminalq"
...
def _configure_root_logger() -> logging.Logger:
    """Configure the "terminalq" logger exactly once and return it."""
    global _configured
...
def get_logger(name: str) -> logging.Logger:
    """Return a child logger namespaced under the "terminalq" root.

    Example: get_logger("fred") -> logger named "terminalq.fred".
