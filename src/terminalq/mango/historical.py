...
``terminalq.mango.fred`` for the same pattern against a different upstream).
"""
...
from terminalq.ext_settings import CACHE_TTL_HISTORY
...
async def get_historical(symbol: str, period: str = "1y", interval: str = "1d") -> dict:
    ...
    convention from `terminalq.mango.fred.get_series`, which returns
