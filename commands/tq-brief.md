...
1. Pull the current picture: call `load_recent_reports` (n=2) for the latest synthesis + snapshot trend. If no reports exist yet, use the latest `record_snapshot` data or run a quick read of the key tools.
...
3. Call `speak` with that script (pass `voice_name` = "$ARGUMENTS" if a voice was given).
